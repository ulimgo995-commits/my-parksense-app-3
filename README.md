...
- **주차장 찾기 페이지**: 진입 시 내 실시간 위치로 지도가 이동하고, 검색 결과 목록은 반경 5km 이내 주차장으로 제한됩니다. 데스크톱은 좌측에 이 목록(추천순/거리순 정렬 — 추천순은 거리와 혼잡도를 함께 계산)을 항상 표시하고, 모바일은 지도 화면에서 "목록 보기" 버튼으로 전체 목록 화면을 오갈 수 있습니다 (구 "내 주변" 탭 기능 통합).
